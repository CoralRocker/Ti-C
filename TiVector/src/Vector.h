...
{
	vector *v;
	v = (vector*)malloc(sizeof(uint8_t*) + sizeof(int));
	v->arr = NULL;
	v->size = 0;
	return v;
}
 ...
 */
void pushBackVector(vector *v, uint8_t value)
{
	uint8_t* temp;
	temp = (uint8_t*)malloc(sizeof(uint8_t)*v->size);
	memcpy(temp, v->arr, sizeof(uint8_t)*v->size);
	free(v->arr);
	v->size++;
	v->arr = (uint8_t*)malloc(sizeof(uint8_t)*v->size);
	memcpy(v->arr, temp, sizeof(uint8_t)*(v->size - 1));
 ...
 */
void insertVector(vector *v, int index, uint8_t value)
{
	uint8_t* temp;
	int i;
	temp = (uint8_t*)malloc(sizeof(uint8_t)*(v->size));
 ...
 */
void assignVector(vector *v, int index, uint8_t value)
{
...
void eraseVector(vector *v, int index)
{
	...
	// Move Information
	memcpy(v->arr, temp, sizeof(uint8_t)*index);
	memcpy(v->arr+index, temp+index+1, sizeof(uint8_t)*(v->size - index));
...
void copyVector(vector *src, vector *dst, size_t size)
{
	...
	{
		free(dst->arr);
		dst->arr = (uint8_t*)malloc(sizeof(uint8_t)*size);
		dst->size = size;
		memcpy(dst, src, size);
	}
	...
	{
		free(dst->arr);
		dst->arr = (uint8_t*)malloc(sizeof(uint8_t)*src->size);
		dst->size = src->size;
 ...
 */
uint8_t atVector(vector *v, int index)
{
...
{
	memset(v->arr, 0, sizeof(uint8_t)*v->size);
	free(v->arr);
	v->arr = NULL;
	v->size = 0;
}
